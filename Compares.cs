    ...
    public class Compares
    {
       ...
       /// <param name="filePath"></param>
        ...
        public void compares(string filePath)
        {
            ...
            {
                MessageBox.Show("Don't fuck with me send a correct path !!!");
                result= false;
            }
                        ...
                        for (int i = 0; i < fileA.Length; i++)
                        {
                            ...
                            for (int j = 0; j < fileB.Length; j++)
                            {
                                if (fileA[i] != "" && fileB[j] != "")
                                {
                                    ...
                                    if (!(fileA[i].Contains("*") || fileA[i].Contains("/")))
                                    {
                                        if (similarity == 0)
                                        {
                                            stringBuilder = stringBuil(stringBuilder, x, k, folderName, folderName1, length, i, j, fileA);
                                            count++;
                                            matchCount++;
        ...
        private StringBuilder stringBuil(StringBuilder stringBuilder, int x , int k, string folderName, string folderName1, int length, int i, int j, string[] fileA)
        {
